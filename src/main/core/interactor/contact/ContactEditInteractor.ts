...
@injectable()
export default class ContactEditInteractor implements Interactor<ContactRequest,ContactResponse>{
    ...
    async execute(request: ContactRequest): Promise<ContactResponse> {
        ...
        let contact = await this._contactEntityGateway.edit(request);
